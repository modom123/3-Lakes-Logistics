...
create policy if not exists carriers_read_own    on public.active_carriers       for select using (id = public.current_carrier_id() or public.is_admin());
create policy if not exists fleet_read_own       on public.fleet_assets          for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists telemetry_read_own   on public.truck_telemetry       for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists drivers_read_own     on public.drivers               for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists hos_read_own         on public.driver_hos_status     for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists eld_read_own         on public.eld_connections       for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists insurance_read_own   on public.insurance_compliance  for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists banking_admin_only   on public.banking_accounts      for select using (public.is_admin());
create policy if not exists sig_read_own         on public.signatures_audit      for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists loads_read_own       on public.loads                 for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists invoices_read_own    on public.invoices              for select using (carrier_id = public.current_carrier_id() or public.is_admin());
create policy if not exists leads_admin_only     on public.leads                 for select using (public.is_admin());
create policy if not exists agent_log_admin_only on public.agent_log             for select using (public.is_admin());
create policy if not exists loads_insert         on public.loads                 for insert with check (true);
create policy if not exists loads_update         on public.loads                 for update using (true);
create policy if not exists loads_delete         on public.loads                 for delete using (true);
create policy if not exists invoices_insert      on public.invoices              for insert with check (true);
create policy if not exists invoices_update      on public.invoices              for update using (true);
create policy if not exists invoices_delete      on public.invoices              for delete using (true);

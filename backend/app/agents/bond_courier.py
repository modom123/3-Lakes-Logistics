...
All messages persist to the `bond_channel` Supabase table so the Bond
Office UI can display a live conversation feed.
"""
...
def send_directive(
    ...
    """Write an IEBC → External Bond directive to bond_channel."""
    sb = get_supabase()
    row = {
...
def poll_external(limit: int = 20) -> list[dict[str, Any]]:
    """Fetch unread External → Internal messages from bond_channel."""
    sb = get_supabase()
    rows = (
...
def thread(limit: int = 50, direction: str = "all") -> list[dict[str, Any]]:
    """Return the full message thread, oldest first."""
    sb = get_supabase()
    q = (
        sb.table("bond_channel")

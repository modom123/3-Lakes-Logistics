...
Integration with hermes-agent:
  Bond is deployed as a hermes-agent persona. The cli-config for his
  profile lives at ~/.hermes/profiles/james_bond/config.yaml and points
  to the Anthropic provider. His system prompt is pulled from prompts.py
  and injected at agent init. This module is the FastAPI-side adapter:
  it gathers intelligence, calls the LLM if ANTHROPIC_API_KEY is
  present, and returns a structured consulting report.
...
# Keys we expect each active agent to have written to memory
...
# Technology domains Bond assesses
_TECH_DOMAINS = [
    "LLM provider",
    "Memory / org brain",
...
def _gather_org_intelligence() -> dict[str, Any]:
    """Pull full org brain + per-agent memories in one pass."""
...
def _agent_coverage_report(by_agent: dict[str, list[dict]]) -> dict[str, Any]:
    """Which agents have written to memory vs. which are silent."""
...
def _assess_lead_pipeline(by_agent: dict[str, list[dict]]) -> dict[str, Any]:
    """Evaluate the health of the lead-generation pipeline."""
    naomi_tier_a = None
    naomi_tier_b = None
    hot_states: list[str] = []
    vance_outcomes: dict = {}
...
def _identify_tech_gaps(
    ...
    """Return a prioritised list of technology gaps Bond has identified."""
    gaps: list[dict[str, Any]] = []

    # Silent agents = coverage gap
    ...
    # Shallow org memory = org brain underused
    if len(org_mems) < 3:
        gaps.append({
            "priority": "HIGH",
            "domain":   "Memory / org brain",
    ...
    # No FMCSA fresh prospects logged
    has_fmcsa = any(
...
def _build_directives(
    ...
    """Translate findings into crisp, ordered Commander directives."""
    directives: list[str] = []
    idx = 1
...
def _executive_summary(
    ...
    """One-paragraph consulting brief for Commander consumption."""
...
def run(payload: dict[str, Any]) -> dict[str, Any]:
    ...
    Payload:
        scope        "full" | "tech" | "ops" | "leads"  (default: "full")
        agent_focus  name of a single agent to zoom in on (optional)
        top_n        how many gaps/directives to surface (default: 5)
    """
    ...
    # Narrow to one agent if requested
    ...
    pipeline = _assess_lead_pipeline(intel["by_agent"])  # always full view

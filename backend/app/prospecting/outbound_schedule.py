"""Step 59: outbound scheduling — only dial/text 9am-5pm local time."""
...
def can_dial(state: str, now_utc: datetime | None = None) -> bool:
    tz = STATE_TZ.get((state or "").upper())
    if not tz:
        return False
    local = (now_utc or datetime.utcnow()).astimezone(ZoneInfo(tz))
    return 9 <= local.hour < 17 and local.weekday() < 5
